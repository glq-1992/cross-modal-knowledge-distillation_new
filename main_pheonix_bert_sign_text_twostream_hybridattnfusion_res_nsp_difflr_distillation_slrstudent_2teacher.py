...
faulthandler.enable()
import utils
from seq_scripts_GSL_bert_text_nsp_distillation_slrstudent_3stage_2teacher import seq_train, seq_eval, seq_feature_generation, seq_eval_translation2gloss
...
class Processor():
    def __init__(self, arg):
        ...
        with open(ch_vocab_filename, encoding='utf-8') as f:
            ...
            # self.gloss_dict[PAD_TOKEN] = len(vocab)
            self.gloss_dict[BOS_TOKEN] = len(vocab) + 1
            self.gloss_dict[EOS_TOKEN] = len(vocab) + 2
    ...
    def loading(self):
        ...
        model = model_class(
            **self.arg.model_args,
            ...
            decoder_arg = self.arg.decoder_arg,
    ...
    def load_checkpoint_weights(self, model, optimizer):
        ...
        if "scheduler_state_dict" in state_dict.keys():
            ...
            schedule_dict = state_dict["scheduler_state_dict"]
            schedule_dict['milestones'] = Counter(self.arg.optimizer_args['step']) # 传入的milestones会在构造函数中经过一个Counter变成_LRScheduler的milestones成员变量
            optimizer.scheduler.load_state_dict(schedule_dict)
    ...
    def load_data(self):
        ...
        for idx, (mode, train_flag) in enumerate(dataset_list):
            ...
            self.dataset[mode] = self.feeder(gloss_dict=self.gloss_dict,tokenizer=tokenizer, **arg)
...
if __name__ == '__main__':
    ...
    sparser = utils.parameters_pheonix_bert_sign_text_twostream_hybridattnfusion_res_nsp_distillation_slrstudent_2teacher.get_parser()
