...
faulthandler.enable()
import utils
from seq_scripts_GSL_bert_text_nsp_distillation_slrstudent_3stage_2teacher_slt import seq_train, seq_eval, seq_feature_generation
...
class Processor():
    def __init__(self, arg):
        ...
        # new label
        ch_vocab_filename = "DatasetFile/dialogue_pheonix/vocab.txt"
        with open(ch_vocab_filename, encoding='utf-8') as f:
            vocab = f.readlines()
            self.gloss_dict = {vocab[i].replace('\n', ''):i+1 for i in range(len(vocab))}

        # new label
        ch_vocab_text_filename = "DatasetFile/dialogue_pheonix/vocab_label_text.txt"
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                # train end2end model
                seq_train(self.data_loader['train'], self.model, self.optimizer,self.device,self.arg.loss_weights,
                          epoch, self.recoder)
                if eval_model:
    ...
    def loading(self):
        ...
        model = model_class(
            **self.arg.model_args,
            ...
            gloss2text_arg = self.arg.gloss2text_args,
    ...
    def load_model_weights(self, model, weight_path):
        ...
        state_dict_2 = torch.load(self.arg.load_weights_multimodalteacher)
        used_weights_multimodalteacher = ['sign_encoder','bert_text','multimodal_fusion','classifier_combine']
        drop_key = set()
        use_key = set()
        for w in used_weights_multimodalteacher:
            for key in state_dict_2['model_state_dict'].keys():
                if key.split('.')[0] == w:
                    use_key.add(key)
        for key in state_dict_2['model_state_dict'].keys():
        ...
        for key in drop_key:
            if state_dict_2['model_state_dict'].pop(key, None) is not None:
                print('Successfully Remove Weights: {}.'.format(key))
            else:
                print('Can Not Remove Weights: {}.'.format(key))
        weights_2 = self.modified_weights(state_dict_2['model_state_dict'], False)

        state_dict_3 = torch.load(self.arg.load_weights_encoder2)
        used_weights_encoder2 = ['sign_encoder_2','conv1d','conv2d','classifier_sign']
        drop_key = set()
        use_key = set()
        for w in used_weights_encoder2:
            for key in state_dict_3['model_state_dict'].keys():
                if key.split('.')[0] == w:
                    use_key.add(key)
        for key in state_dict_3['model_state_dict'].keys():
            if key not in use_key:
                drop_key.add(key)
        for key in drop_key:
            if state_dict_3['model_state_dict'].pop(key, None) is not None:
                print('Successfully Remove Weights: {}.'.format(key))
            else:
                print('Can Not Remove Weights: {}.'.format(key))
        weights_3 = self.modified_weights(state_dict_3['model_state_dict'], False)

        # weights = self.modified_weights(state_dict['model_state_dict'])
        model.load_state_dict(weights, strict=False)
        model.load_state_dict(weights_2, strict=False)
        # model.sign_encoder.load_state_dict(weights_2, strict=False)
        # model.bert_text.load_state_dict(weights_2, strict=False)
        # model.multimodal_fusion.load_state_dict(weights_2, strict=False)
        # model.classifier_combine.load_state_dict(weights_2, strict=False)
        # for name,parameters in model.sign_encoder_2.named_parameters():
        #     if name == 'temporal_model.rnn.bias_ih_l1_reverse':
        #         print(name,':',parameters)
        # print('--------')
        model.load_state_dict(weights_3,strict = False)
        # for name,parameters in model.sign_encoder_2.named_parameters():
        #     if name == 'temporal_model.rnn.bias_ih_l1_reverse':
        #         print(name,':',parameters)
...
if __name__ == '__main__':
    ...
    sparser = utils.parameters_pheonix_bert_sign_text_twostream_hybridattnfusion_res_nsp_distillation_slrstudent_2teacher_slt.get_parser()
